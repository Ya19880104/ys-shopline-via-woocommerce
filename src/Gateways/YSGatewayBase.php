...
abstract class YSGatewayBase extends WC_Payment_Gateway {
    ...
    public function handle_pay_redirect() {
        ...
        YSLogger::debug( 'handle_pay_redirect triggered', array(
        ...
        if ( ! $order || $order->get_order_key() !== $key ) {
            YSLogger::error( 'Invalid order in pay redirect', array(
                'order_id' => $order_id,
                'key'      => $key,
            ) );
        ...
        YSLogger::debug( 'Checking gateway match', array(
        ...
        if ( ! $next_action ) {
            ...
            YSLogger::debug( 'No next action, redirecting to thank you page' );
            wp_safe_redirect( $this->get_return_url( $order ) );
            exit;
        }

        // Render 3DS/redirect page
        YSLogger::debug( 'Rendering 3DS page' );
