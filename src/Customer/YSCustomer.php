...
class YSCustomer {
	...
	public function get_or_create_customer_id( $user_id ) {
		...
		// 建立新的 Customer
		if ( ! $this->api ) {
			YSLogger::error( 'Cannot create customer: API not configured' );
			return false;
		}
		...
		if ( is_wp_error( $response ) ) {
			YSLogger::error( 'Failed to create customer: ' . $response->get_error_message(), array(
				'user_id' => $user_id,
			) );
			return false;
		}
		...
		if ( isset( $response['customerId'] ) ) {
			...
			YSLogger::info( 'Customer created', array(
				'user_id'     => $user_id,
	...
	public function get_payment_instruments( $user_id, $force_fresh = false ) {
		...
		if ( is_wp_error( $response ) ) {
			YSLogger::error( 'Failed to get payment instruments: ' . $response->get_error_message(), array(
				'user_id'     => $user_id,
	...
	public function maybe_sync_tokens_from_api() {
		...
		YSLogger::info( 'Syncing payment instruments to WC Tokens', array(
			'user_id' => $user_id,
	 ...
	 * 從付款工具建立 WC Token
	 *
	 * API 文件欄位: instrumentId, instrumentCard.brand, instrumentCard.last,
	 *               instrumentCard.expireMonth, instrumentCard.expireYear
	 *
	 * @param int   $user_id    WordPress 用戶 ID
	...
	private function create_wc_token_from_instrument( $user_id, $instrument ) {
		...
		if ( empty( $instrument_id ) || ! is_string( $instrument_id ) ) {
			return false;
		}

		// 檢查是否已存在
		$existing = $this->get_wc_token_by_instrument_id( $user_id, $instrument_id );
		if ( $existing ) {
			return $existing;
		}
	...
	public function sync_tokens_from_api( $user_id ) {
		...
		$customer_id = $this->get_customer_id( $user_id );
		if ( ! $customer_id ) {
			YSLogger::debug( 'sync_tokens_from_api: No customer ID', array(
				'user_id' => $user_id,
			) );
			return 0;
		}
		...
		if ( empty( $instruments ) ) {
			YSLogger::debug( 'sync_tokens_from_api: No instruments from API', array(
				'user_id'     => $user_id,
				'customer_id' => $customer_id,
			) );
			return 0;
		...
		YSLogger::info( 'Tokens synced from API', array(
			'user_id'      => $user_id,
	...
	public function ajax_sync_cards() {
		...
		YSLogger::info( 'Cards synced from API', array(
			'user_id'      => $user_id,
			'total'        => count( $instruments ),
			'synced'       => $synced_count,
		) );
